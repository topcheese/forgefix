...
`
	...
	tmpDir := t.TempDir()
	configPath := filepath.Join(tmpDir, "forgefix.yaml")
...
func TestLoadConfigFromPathWithFailureDecay(t *testing.T) {
	yamlContent := `
project: "Failure Decay Test"
...
`
	...
	if loaded.Config.FailureDecaySeconds != 30 {
		t.Errorf("expected FailureDecaySeconds=30, got %d", loaded.Config.FailureDecaySeconds)
	}
}

func TestFailureDecaySecondsDefaultsTo15(t *testing.T) {
	yamlContent := `
project: "Default Decay Test"
global_timeout_seconds: 60
languages:
  go_mod:
    root_anchor: "go.mod"
...
`
	...
	if loaded.Config.FailureDecaySeconds != 0 {
		t.Errorf("expected FailureDecaySeconds=0 when unset, got %d", loaded.Config.FailureDecaySeconds)
	}
}
